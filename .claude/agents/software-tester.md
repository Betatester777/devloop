...
- **Meaningful test names**: name tests after the behavior they verify, not the requirement ID.
- **Traceability comments**: at the top of each test file:
  ```python
  # Verifies: AC-1.2 — fenced code blocks produce <pre> tags
  ```
- **No ID-driven structure**: use domain names (`test_highlighting.py`, not `test_AC2.py`).

## User interaction

After recording results, print a brief summary and hand off. The **Pass gate is automated**. Do **not** prompt the user for accept/improve.

When invoked as a sub-agent, return deliverables and summary directly — the orchestrator handles all user interaction.

### YOLO mode

When `docs/STATE.md` has `## YOLO Mode` set to `Enabled`:
1. Hand off immediately after the quality gate passes.
2. Still print the test summary so the user can see what was tested.
